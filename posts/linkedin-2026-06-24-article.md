# LinkedIn Article — Week 2: Full Founder Story Republished

**Post date:** Wednesday, June 24, 2026
**Format:** LinkedIn Article (long-form, not a regular post) + companion feed post
**Version:** v4 — final, ready to publish

> **v4 changelog (vs v3):**
> - [P33] Decided to keep original snappy version: *"The international ed dream wasn't wrong. The format was."* (Tyler chose snap over softening — the "format was" phrasing already honors int'l ed enough)
> - All 3 image slots locked in (see asset list at bottom)
> - All TBD/⚠️ flags resolved

---

## How to publish as a LinkedIn Article

1. LinkedIn home page → click **Write Article**
2. **Title:** `How I ended up running a card show calendar in Hawaii`
3. **Hero image:** `posts/assets/article-2026-06-24/hero-first-ever-vending.jpeg` (you + friend at card shop, throwing shakas)
4. **Body:** Paste the essay below. `[IMAGE-N]` markers in the body indicate inline image spots
5. **At the top of the body**, italic + small:
   > *Originally published at [hawaiicardshows.com/founders-note](https://hawaiicardshows.com/founders-note)*
6. **Publish**

---

## Companion LinkedIn Post (post this immediately after the Article publishes)

> Last week I shared the short version of how I went from international ed to running a card show calendar in Hawaii.
>
> A bunch of folks asked for the full story — so here it is.
>
> Five years in digital communities (web3, gaming, AI) before stumbling into a Saturday morning card show that rerouted everything I thought I was doing.
>
> Full essay → [link to LinkedIn article]
>
> #BuildingInPublic #FoundersJourney #Hawaii

---

## Full Article Body (paste into the LinkedIn Article composer)

*Originally published at [hawaiicardshows.com/founders-note](https://hawaiicardshows.com/founders-note)*

**[HERO IMAGE — `posts/assets/article-2026-06-24/hero-first-ever-vending.jpeg`]**

In 2020 I was on a path I thought would last my whole life.

I was working in international education at the time, designing study abroad programs for students. The mission felt important to me — get people across borders, build relationships between cultures that didn't usually have a reason to know each other, nurture the next generation of leaders who actually understood the world beyond their own slice of it. The world felt fractured, and the answer felt obvious: get people out of their bubbles and put them face-to-face with people whose lives looked nothing like their own. I'd just been accepted into a master's program in the field. I figured this was the career I'd be in for the long haul.

Then COVID hit.

International ed didn't slow down. It fell off a cliff. Nobody was traveling. Programs froze. Universities pulled funding. My wife and I started talking seriously about moving back to where she's from — Hawaii. The future I'd planned suddenly didn't fit the world we were living in. Building a career around shipping students across continents wasn't going to work when shipping anyone across anything had stopped.

So I did what a lot of people did during that strange in-between period. I started pulling on the other threads in my life — letting curiosity lead the way.

### Finding community where I didn't expect it

Around the same time, I was tinkering with digital collectibles. I was mostly curious — the whole space was loud and weird and a little embarrassing to admit interest in. But I kept showing up in Discord servers attached to projects I liked, and I started noticing something I wasn't expecting.

People from the Philippines, the US, Germany, China, Brazil, Vietnam — they were forming real communities around shared interests. Late-night calls. Joint projects. Inside jokes that lived for months. People showing up for each other across time zones in ways that felt, honestly, more real than a lot of the in-person communities I'd been a part of.

**Everything I had been trying to engineer through formal study abroad programs was happening organically, at scale, in the digital space.**

Strangers from opposite sides of the planet were becoming actual friends. Without funding, without bureaucracy, without an institution in the middle of it. Just because they cared about the same things. (And sure, money was driving a lot of it. But the communities forming around it were real.)

That shook something loose for me.

If the goal was cross-cultural relationships, the format wasn't the only way to get there. Maybe wasn't even the best way anymore.

So I followed the thread.

### Five years building things across borders

What started as curiosity became a career. I went on to help a Malaysian/Australian founder build a community and launch a discovery platform. I helped create a gaming distribution platform. I was recruited by a triple-A game studio to help launch a title pioneering new tech in an established space. Then I worked with a Philippines-based team innovating at the intersection of AI and crypto.

It was an incredible run. I learned more in those five years than I ever would have in a classroom — about how communities actually form, how trust builds, how stories spread, how products live or die based on whether real people show up for them. I also learned how to build teams across time zones, how to spot good talent when I saw it, and what it takes to ship a product polished enough that people actually want to use it. All on the job. All in the trenches with founders chasing big bets.

But somewhere around year four, as the world started reopening and the shockwave from COVID started to dissipate, I started noticing something else.

### What the digital era couldn't replace

People were tired.

Not tired of the internet. Tired of *only* having the internet.

Five years of Discord calls and group chats and livestreams and remote meetings had been an extraordinary substitute, but it had never been a replacement. The thing that screen-mediated community could never fully give people was the same thing the international ed programs were ultimately about: **being in the same room, looking each other in the eye, being part of the same moment.**

**Humans crave human-to-human contact.**

Real, in-person, walk-up-and-shake-a-stranger's-hand contact. The kind you can't fake with a webcam.

I wasn't sure what to do with that observation. I just sat with it.

### An accidental Saturday morning

The next chapter happened almost by accident.

I was getting into golf. A friend invited me to play a round at a course on Oahu, and that same morning at the same course there happened to be a card show. I figured I'd swing by before the round started. I went with him and his two daughters and immediately got hooked.

**[IMAGE-1 — `posts/assets/article-2026-06-24/image-1-pearl-country-club-show-floor.png` (Pearl at Kalauao show floor, April 2025 — the moment that started it all)]**

I hadn't seen that many people gathered in person around a shared hobby in years. Kids. Uncles. Aunties. Pros. Locals and visitors. Restaurant owners next to valets next to business owners — strangers who probably wouldn't have stopped each other on the street, walls down because of a shared love for cardboard.

The energy was electric. Not nostalgic — *present*. Not a relic from a previous decade. This was a community that had been quietly building the whole time the internet had been telling itself it had replaced everything.

I spent the next two months going deep.

I visited every card shop on the island. Made friends with shop owners. A vendor friend invited me to set up at a show — that pulled me in even further. I started to see the patterns: which shows had the right energy, which organizers actually cared about their community, which shops were quietly anchoring entire neighborhoods of collectors.

Then I noticed the problem.

### The problem nobody had solved

Finding card shows in Hawaii was nearly impossible.

I had to reactivate Instagram just to find them. I was answering "when's the next card show?" in Facebook group chats every week. On a trip to Detroit, I tried to scope the mainland scene and it was even worse — one of the best shows I found there had no website, no Instagram page, no flyer online. A vendor from out of state had taken a photo of a printed flyer and posted it to a group chat. **That was how you found out.**

This was a community packing rooms, moving real money, building real friendships — and the digital layer that should have been pulling all of that together was barely there.

It struck me as the exact mirror image of what I'd just spent five years building. In web3 and gaming, I'd been helping projects with global digital communities solve for *physical* presence — meetups, conventions, IRL events. Here was a community that already had the physical presence solved at a level most digital-native projects would kill for. But the digital layer that should have been amplifying it was almost nonexistent.

I figured I'd try to fix that.

### What started as a rolodex

I'm not technical. But I've spent enough time in QA and marketing across different products to know what a polished one looks like — and AI has leveled up enough in the last 18 months that the gap between "I have an idea" and "I have a working website" has collapsed in ways most people haven't really sat with yet.

What started as a single session trying to organize my own card-show contacts turned into a two-week build of a real community platform.

[Hawaii Card Shows](https://hawaiicardshows.com) is the result. The free community calendar for trading card events across the Hawaiian Islands. Every show, every trade night, every shop on every major island, in one place.

It's free to use. Free to list. Built by collectors, for collectors. Submitted by the community, maintained by me and a small group of organizers and shop owners I trust.

### Where we are three months in

I launched it quietly in early April 2026. No paid marketing. No press releases. No paid acquisition of any kind. Just submitted it to a few SEO directories, posted it in a handful of local group chats, and let the community find it organically.

- **9,200+** unique visitors
- **33,000+** pageviews
- **77** events tracked across all four major Hawaiian islands
- **72%** of traffic from organic Google search
- **144** newsletter subscribers
- Collectors finding the site from Honolulu to Los Angeles to Seattle to Phoenix

Zero paid marketing. All organic, all word-of-mouth, all SEO.

**[IMAGE-2 — `posts/assets/article-2026-06-24/image-2-website-homepage.png` (the homepage you're driving traffic to)]**

And those numbers don't fully capture it. A festival weekend in early June — Keep It Aloha at SALT Kaka'ako on Friday, West Side Cardshow at the Kroc Center on Saturday and Sunday — drew **over 14,000 combined attendees** across the two shows. UFC champion Max Holloway showed up to vend his own personal collection at the West Side show. Real people, real gathering, in real life.

### What this is really about

**The international ed dream wasn't wrong. The format was.**

The thing I was chasing in 2020 — communities that genuinely connect people across difference — was right there the whole time. It just didn't look like a classroom in Spain. It looked like a folding table in a Kaka'ako warehouse with a 60-year-old retired teacher and a 12-year-old kid arguing over a Charizard.

If I've learned one thing across this whole arc — from study abroad to Discord servers to building products with founders in Kuala Lumpur and Manila to standing in a parking lot watching strangers become friends over trading cards — it's this:

**Communities are not built by content. They are built by gathering.**

Digital tools should make gathering easier. They should not try to replace it.

That's what Hawaii Card Shows is. A digital tool that exists to make the in-person gatherings easier to find.

### What's next

I'll keep building. More recap content. More tools for organizers. Some collaborations with shops and creators I'm pretty excited about. The newsletter is becoming its own thing, and the mainland audience is starting to ask about visiting.

If you're in the trading card space, building community, or working at the intersection of digital tools and in-person experiences — I'd love to connect. And if you're in Hawaii, come find me at a show. 🤙

🔗 [hawaiicardshows.com](https://hawaiicardshows.com)

*— Tyler*

---

## 🖼️ Image asset list (locked in)

| Spot | File | Description |
|---|---|---|
| **HERO** | `posts/assets/article-2026-06-24/hero-first-ever-vending.jpeg` | Tyler + friend at card shop, throwing shakas — captures the energy of "I went deeper" |
| **IMAGE-1** (Pearl Country Club moment) | `posts/assets/article-2026-06-24/image-1-pearl-country-club-show-floor.png` | The packed Pearl at Kalauao show floor, April 2025 — the Saturday morning that started it all |
| **IMAGE-1 alt** (IG post variant, if Tyler prefers context) | `posts/assets/article-2026-06-24/image-1-pearl-country-club-ig-post.png` | Same photo but with the @hawaiicollectiblesshow IG sidebar visible — adds proof/context |
| **IMAGE-2** (website/calendar screenshot) | `posts/assets/article-2026-06-24/image-2-website-homepage.png` | The hawaiicardshows.com homepage — what readers find when they click through |

---

## ⚙️ Streamlined feedback system (proposed)

Going forward, every draft I send you will include:

1. **Paragraph numbers** `[P1]`, `[P2]`, etc. — so you can reference specific sections without copy-pasting
2. **A changelog block at the top** — shows what changed from the previous version, version-numbered (v1, v2, v3...)
3. **TBD/⚠️ flags inline** — so you can spot pending decisions at a glance
4. **An asset list table at the bottom** — image picks, links, anything external

Your feedback style is already great. You usually say things like *"P3 is too choppy"* or *"after P9 add a parenthetical about money."* That maps cleanly onto the numbered-paragraph system. You can also just paste a sentence you want changed and the change you want — that works too.

If you want a step further, I can also add a **section-level table of contents** at the top so you can navigate by section name. Let me know if that's useful or overkill.
